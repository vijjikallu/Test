...
import com.project.util.CustomErrorType;
...
public class EmployeeController {
	...
	// Never code against a specific logging framework. Always code against use slf4j api for loose coupling
	...
	public ResponseEntity<?> createEmployee(@RequestBody Employee employee, UriComponentsBuilder ucBuilder) {
		...
		if (employeeService.isEmployeeExists(employee)) {
			...
			return new ResponseEntity(new CustomErrorType("Unable to create. A Employee with name " + employee.getName() + " already exist."), HttpStatus.CONFLICT);
		}
	...
	public ResponseEntity<?> updateEmployee(@PathVariable("id") long id, @RequestBody Employee employee) {
		...
		if (currentEmployee == null) {
			...
			return new ResponseEntity(new CustomErrorType("Unable to upate. Employee with id " + id + " not found."), HttpStatus.NOT_FOUND);
		}
